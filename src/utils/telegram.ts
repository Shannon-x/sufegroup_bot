...
const DEFAULT_REJOIN_COOLDOWN_SECONDS = 60;
 ...
 * the member is already out of the group. For a classic kick, however, lifting
 * the ban is part of the promised outcome: without it a legitimate user cannot
 * rejoin and retry verification. An unban failure therefore propagates to the
 * caller's compensation queue instead of silently becoming a permanent ban.
 *
...
export async function kickUser(
  ...
  chatId: number,
  userId: number,
  rejoinCooldownSeconds: number = DEFAULT_REJOIN_COOLDOWN_SECONDS
): Promise<void> {
