...
#### VERIFICATION_REJOIN_COOLDOWN_SECONDS
- **默认值**: `60`
- **取值范围**: `0`-`86400`
- **说明**: 超时操作为 `kick` 时，用户被移出后多久可以重新入群。`0` 表示立即解除封禁并允许重新验证；正数会按 Telegram 规则自动提高到至少 35 秒
- **建议**: 默认保持 `60` 秒以阻断自动账号立即重入；更重视无等待恢复时可设为 `0`，攻击频繁时可提高到 `300`
